...
while(quantidadeImagens<=10):
	...
	cv2.imwrite("../resultados/orb-freak/img"+str(quantidadeImagens)+".jpg", img3)
	vet_matches.append(len(matches))
	vet_corretos.append(acertos)
	mycursor = mydb.cursor()
	sql = "INSERT INTO orb_freak(Nome, Matches, Correto, ImgReferente) VALUES (%s, %s, %s, %s)"
	valor = ("Orb-freak"+str(quantidadeImagens), len(matches), acertos, "img"+str(quantidadeImagens)+".jpg")
