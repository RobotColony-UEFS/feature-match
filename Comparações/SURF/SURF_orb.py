...
while(quantidadeImagens<=10):
	...
	cv2.imwrite("../resultados/surf-orb/img"+str(quantidadeImagens)+".jpg", img3)
	vet_matches.append(len(matches))
	vet_corretos.append(acertos)
	mycursor = mydb.cursor()
	sql = "INSERT INTO surf_orb(Nome, Matches, Correto, ImgReferente) VALUES (%s, %s, %s, %s)"
	valor = ("Surf-Orb"+str(quantidadeImagens), len(matches), acertos, "img"+str(quantidadeImagens)+".jpg")
...
valor2 = ("surf_orb", media_matches, desvio_matches, media_corretos, desvio_corretos, porcentagem)
