...
while(quantidadeImagens<=10):
	...
	cv2.imwrite("../resultados/surf-sift/img"+str(quantidadeImagens)+".jpg", img3)
	vet_matches.append(len(matches))
	vet_corretos.append(acertos)
	mycursor = mydb.cursor()
	sql = "INSERT INTO surf_sift(Nome, Matches, Correto, ImgReferente) VALUES (%s, %s, %s, %s)"
	valor = ("Surf-Sift"+str(quantidadeImagens), len(matches), acertos, "img"+str(quantidadeImagens)+".jpg")
...
valor2 = ("surf_sift", media_matches, desvio_matches, media_corretos, desvio_corretos, porcentagem)
