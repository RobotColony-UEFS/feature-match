...
while(quantidadeImagens<=13):
	...
	cv2.imwrite("../resultados/shiTomasi-orb/img"+str(quantidadeImagens)+".jpg", img3)
	vet_matches.append(len(matches))
	vet_corretos.append(acertos)
	mycursor = mydb.cursor()
	sql = "INSERT INTO shiTomasi_orb(Nome, Matches, Correto, ImgReferente) VALUES (%s, %s, %s, %s)"
	valor = ("ShiTomasi-Orb"+str(quantidadeImagens), len(matches), acertos, "img"+str(quantidadeImagens)+".jpg")
...
valor2 = ("shiTomasi_orb", media_matches, desvio_matches, media_corretos, desvio_corretos, porcentagem)
