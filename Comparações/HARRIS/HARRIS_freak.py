...
while(quantidadeImagens<=13):
	...
	cv2.imwrite("../resultados/harris-freak/img"+str(quantidadeImagens)+".jpg", img3)
	vet_matches.append(len(matches))
	vet_corretos.append(acertos)
	mycursor = mydb.cursor()
	sql = "INSERT INTO harris_freak(Nome, Matches, Correto, ImgReferente) VALUES (%s, %s, %s, %s)"
	valor = ("Harris-Freak"+str(quantidadeImagens), len(matches), acertos, "img"+str(quantidadeImagens)+".jpg")
...
valor2 = ("harris_freak", media_matches, desvio_matches, media_corretos, desvio_corretos, porcentagem)
