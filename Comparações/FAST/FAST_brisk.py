...
while(quantidadeImagens<=10):
	...
	cv2.imwrite("../resultados/fast-brisk/img"+str(quantidadeImagens)+".jpg", img3)
	vet_matches.append(len(matches))
	vet_corretos.append(acertos)
	mycursor = mydb.cursor()
	sql = "INSERT INTO fast_brisk(Nome, Matches, Correto, ImgReferente) VALUES (%s, %s, %s, %s)"
	valor = ("Fast-Brisk"+str(quantidadeImagens), len(matches), acertos, "img"+str(quantidadeImagens)+".jpg")
...
valor2 = ("fast_brisk", media_matches, desvio_matches, media_corretos, desvio_corretos, porcentagem)
